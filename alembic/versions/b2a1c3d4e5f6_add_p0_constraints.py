...
def upgrade() -> None:
    op.execute("UPDATE offers SET current_price = 0.01 WHERE current_price <= 0")
    op.execute("UPDATE price_changes SET price = 0.01 WHERE price <= 0")
    op.execute("UPDATE price_history SET price = 0.01 WHERE price <= 0")
